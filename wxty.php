...
const KANKAN_API_BASE = 'https://kapi.kankanews.com';
const KANKAN_API_VERSION = '2.42.23'; // 与官网当前版本保持一致
const KANKAN_API_SECRET = '28c8edde3d61a0411511d3b1866f0636';
const KANKAN_UA = 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.5845.97 Safari/537.36 SE 2.X MetaSr 1.0';

function kankanApiGet($path, $params = []) {
    $t = time();
    $nonce = getnonce(8);
    $uuid = getuuid();
    $all = $params;
    ...
    $all['platform'] = 'pc';
    $all['timestamp'] = $t;
    $all['version'] = KANKAN_API_VERSION;
    ksort($all);
    $signStr = '';
    foreach ($all as $k => $v) {
        if ($v === null) continue;
    ...
    $sign = md5(md5($signStr . KANKAN_API_SECRET));
    ...
    $headers = [
        ...
        'Platform: pc', 'Version: ' . KANKAN_API_VERSION, "Timestamp: {$t}", "Sign: {$sign}",
        'Origin: https://live.kankanews.com', 'Referer: https://live.kankanews.com/',
        'User-Agent: ' . KANKAN_UA,
        "X-Forwarded-For: {$clientIp}", "Client-Ip: {$clientIp}",
    ];
    $url = toLocalProxyUrl(KANKAN_API_BASE . $path . '?' . http_build_query($params));
    $ch = curl_init();
    curl_setopt_array($ch, [
...
function fetchLiveAddress($channelId) {
    $detail = kankanApiGet('/content/pc/tv/channel/detail', ['channel_id' => $channelId]);
    $addr = (string)($detail['result']['live_address'] ?? '');
    if ($addr !== '') return $addr;

    $programs = kankanApiGet('/content/pc/tv/programs', ['channel_id' => $channelId, 'date' => date('Y-m-d')]);
    $list = $programs['result']['programs'] ?? [];
    if (!$list) return '';

    $now = time();
    $current = $next = $last = null;
    foreach ($list as $p) {
        $s = (int)($p['start_time'] ?? 0);
        $e = (int)($p['end_time'] ?? 0);
        if ($s <= $now && $now < $e) { $current = $p; break; }
        if ($s > $now && ($next === null || $s < (int)$next['start_time'])) $next = $p;
        if ($e <= $now && ($last === null || $e > (int)$last['end_time'])) $last = $p;
    }
    $pick = $current ?: $next ?: $last;
    if (!$pick) return '';

    $program = kankanApiGet('/content/pc/tv/program/detail', ['channel_program_id' => $pick['id']]);
    return (string)($program['result']['channel_info']['live_address'] ?? '');
}
...
function getLiveUrl($channelId, $forceRefresh = false, &$cacheHit = null) {
    ...
    $addr = fetchLiveAddress($channelId);
    if ($addr === '') return false;
    $liveUrl = rsaDecrypt($addr);
